...
import { UtensilsCrossed, Clock, ShoppingBag, Plus, Minus, CheckCircle2, X, ChefHat, Bell, Flame, BarChart3, Settings, Package, ClipboardList, AlertCircle, Timer, TrendingUp } from "lucide-react";

const C = { red: "#E74C3C", dark: "#1A1A2E", card: "#16213E", sidebar: "#0F3460", accent: "#E94560", white: "#FFFFFF", gray: "#94A3B8", light: "#F1F5F9", cream: "#FFF8F0", navy: "#0F172A" };

const liveOrders = [
  { id: "R-4521", items: ["Bruschetta x2", "Tagliatelle"], table: "Stolik 5", time: "3 min temu", total: 114, status: "new" },
  { id: "R-4520", items: ["Risotto", "Carpaccio"], table: "Stolik 8", time: "8 min temu", total: 94, status: "preparing" },
  { id: "R-4519", items: ["Filetto 300g", "Tiramisù"], table: "Dostawa", time: "12 min temu", total: 130, status: "preparing" },
  { id: "R-4518", items: ["Spaghetti Arrab.", "Panna Cotta", "Aperol x2"], table: "Stolik 2", time: "22 min temu", total: 122, status: "ready" },
  { id: "R-4517", items: ["Salmone", "Bruschetta"], table: "Stolik 12", time: "35 min temu", total: 100, status: "served" },
];

const menuItems = [
  { name: "Bruschetta al Pomodoro", cat: "Antipasti", price: 28, active: true, popular: true },
  { name: "Carpaccio di Manzo", cat: "Antipasti", price: 42, active: true, popular: true },
  { name: "Tagliatelle al Tartufo", cat: "Pasta", price: 58, active: true, popular: true },
  { name: "Spaghetti all'Arrabbiata", cat: "Pasta", price: 38, active: true, popular: false },
  { name: "Risotto ai Funghi Porcini", cat: "Drugie dania", price: 52, active: true, popular: true },
  { name: "Filetto di Manzo 300g", cat: "Drugie dania", price: 98, active: true, popular: true },
  { name: "Salmone alla Griglia", cat: "Drugie dania", price: 72, active: true, popular: false },
  { name: "Tiramisù Classico", cat: "Dolci", price: 32, active: true, popular: true },
  { name: "Panna Cotta", cat: "Dolci", price: 28, active: true, popular: false },
  { name: "Aperol Spritz", cat: "Napoje", price: 28, active: false, popular: true },
];
...
const topNav = [
  ...
  { id: "settings", label: "Ustawienia", icon: <Settings className="w-3.5 h-3.5" /> },
];
        ...
        {tables.map(t => {
          ...
          return (
            <div key={t.id} className="p-2.5 rounded-xl text-center" style={{ background: st.bg, border: `2px solid ${st.border}40` }}>
              ...
              {t.status === "occupied" && <span className="text-[8px] font-mono block mt-0.5" style={{ color: C.red }}>{(t as any).order}</span>}
              {t.status === "reserved" && <span className="text-[8px] block mt-0.5" style={{ color: "#F59E0B" }}>{(t as any).time}</span>}
            </div>
          );
        })}
      </div>
...
function SettingsPage() {
  return (
    <div className="p-3 space-y-2">
      <span className="text-xs font-bold" style={{ color: C.navy }}>Ustawienia restauracji</span>
      {[
        { label: "Nazwa", value: "Trattoria Bella" },
        { label: "Adres", value: "ul. Nowy Świat 42, Warszawa" },
        { label: "Godziny", value: "12:00 – 23:00" },
        { label: "Dostawa", value: "Aktywna (30 min)" },
        { label: "Min. zamówienie", value: "50 zł" },
        { label: "Płatności", value: "Karta, BLIK, gotówka" },
      ].map((s, i) => (
        <div key={i} className="flex items-center justify-between p-2.5 rounded-xl" style={{ background: C.white }}>
